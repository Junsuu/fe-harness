...
| **③ refine** | 지적을 어떻게 고칠지 제시 | 적용 여부는 사람 |
...
반대로 **셀 수 있다고 차단하지는 않는다.** 현업 저장소 4곳의 정상 `.tsx` 중
**5~31%가 컴포넌트 2개 이상**이었고, 걸리는 것들이 아이콘 배럴 같은 정당한
패턴이었다. 파일 길이도 마찬가지로 데이터 테이블·타입 정의·SVG처럼 정당하게
긴 경우가 실재한다.

**그래서 차단하는 훅이 하나도 없다.** 세는 것은 신호로만 쓰고, 그 개수가
문제인지는 `review`가 읽고 판단한다.
