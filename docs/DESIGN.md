...
- **4 · 품질 게이트** — `Stop` · 안전망 ✅
  타입체크·테스트 통과 전 종료 차단. **추론하지 않는다** — 차단하는 훅이라
  확신 없는 추론으로 개발을 막으면 안 된다. 설정에 명시했을 때만 돈다.
  변경이 없으면(`git status` 가 비면) 돌지 않는다
- ~~**5 · flag props**~~ — **폐기.** 이유는 아래 「의도적으로 하지 않는 일」
- **6 · 린트 피드백** — `PostToolUse` · 피드백 루프 ✅
  `--fix` 후 남은 것만 stderr
- **7 · 규칙 주입** — `SessionStart` · 규칙 희석 + **예방** ✅
  stdout 이 컨텍스트로 가는 3개 이벤트 중 하나.
  컴팩션 후 재주입이 원래 목적이었지만, **유일한 예방 채널**이기도 하다.
  실제 임계값을 그대로 알려준다. 2줄을 넘기지 않는다

**P1-4 와 P1-6 은 성격이 다르다 — 이것만 퀄리티를 직접 올린다.**
분량·컴포넌트 게이트는 구조 열화 **예방**이지 "좋은 코드 쓰기"가 아니다.
반면 타입 에러와 lint 에러는 취향이 아니라 **틀린 코드**다. 셀 수 있으면서
퀄리티를 올리는 유일한 영역이고, 4장 경계선의 확실한 왼쪽이다.
...
│       ├── lint-feedback.sh     ← P1-6 ✅ 경고
│       ├── gate-stop.sh         ← P1-4 ✅ 반려 (Stop)
│       └── reinject.sh          ← P1-7 ✅ 주입 (SessionStart)
...
- ✅ **7** · 조정 (임계값 · 예외 경로)
  현업 저장소 5개 실측으로 `maxEditLines` 150 → 80
- ✅ **7.5** · **P1 마무리** — 린트 피드백 · 품질 게이트 · 규칙 주입
  훅으로 갈 수 있는 퀄리티 향상의 최대치
...
- [x] **서로 다른 저장소 2개**에서 동작 확인 — 모노레포(turborepo) + 단일 패키지 Next.js
...
표가 아니라 목록으로 쓴다. 셀이 길어지면 표는 원문으로 못 읽는다.
각 항목은 **무엇이 걸렸나 → 어떻게 했나** 두 줄이다.

### 2026-08-18

**255줄 `.tsx` `Write` 가 반려됐다.**
파일이 실제로 만들어지지 않았고, 모델은 우회를 시도하지 않았다.
→ 변경 없음. **이게 기준선이다.**

**컴포넌트 2개짜리 `Write` 반려 → 시킨 대로 두 파일로 쪼개니 통과.**
→ 변경 없음. **반려가 막다른 골목이 아니라는 것**을 확인했다. 훅이 제시한
대안이 안 통하면 모델은 결국 우회를 찾는다.

**`Edit`로 두 번째 컴포넌트를 끼워 넣으니 파일은 써지고 경고만 나왔다.**
의도대로지만 **되돌리는 건 모델의 선택**에 달려 있다.
→ 변경 없음. 실제로 얼마나 되돌리는지 관찰이 필요하다.

**현업 저장소 5개 실측 — `maxEditLines` 150 이 수정 분포의 p98.5 라 사실상
안 걸리고 있었다.** 새 파일(p95)보다 헐렁해서 1장의 처방과 반대로 작동했다.
→ **150 → 80** (p96~97). 안 걸리면 60 → 40 으로 조인다.
`maxNewFileLines` 250 은 p95 라 유지.

**주문 목록 페이지 실험 — 187줄이라 분량 게이트는 안 걸리고 컴포넌트 8개로
걸렸다.**
→ 변경 없음. **두 게이트가 상보적**임이 확인됐다. 전형적 페이지는 줄 수보다
컴포넌트 개수로 먼저 비대해진다.

**위 실험의 해소 — 반려 1회, 재시도 0회, 연쇄 반려 없음.**
stderr 세 줄만 보고 9개 파일로 쪼갰고 헤맨 흔적이 없다.
→ 변경 없음. **절차 스킬 보류 유지.** "쪼개는 법을 모른다"는 문제는 없었다.

**다만 5~7줄짜리 표현 컴포넌트 3개가 각각 파일로 밀려났다.**
`StatusBadge` 5줄, `EmptyState` 7줄, `LoadingState` 7줄. 총 줄 수 +10%,
주 파일의 12%가 import 가 됐다.
→ 변경 없음. **한 번의 관찰로 게이트를 완화하지 않는다.** 반복되면
`maxComponentsPerFile` 2 를 검토한다.

**게이트는 분할의 타당성을 못 본다.**
`OrdersPageTop` / `OrdersPageBottom` 처럼 줄 수만 맞춘 무의미한 분할도
똑같이 통과시킨다.
→ 훅으로 메울 수 없다(4장 경계선의 오른쪽). 예방은 주입, 검증은 리뷰 시점.

**증상 ① 을 "해결"로 적었던 게 과대평가였다.**
게이트는 도구 호출 하나의 크기를 볼 뿐 턴 전체를 안 본다. 240줄 × 10파일은
전부 통과한다. 게다가 ⑤ 를 고치는 방식(쪼개서 각각 `Write`)이 턴 내 호출 수를
늘려 ① 을 악화시킨다.
→ 턴 단위 누적은 **만들지 않기로 결정.** 리팩터링 턴과 오타 수정 턴의 적정치가
달라 임계값이 존재할 수 없다. README 「다루지 않는 것」에 명시.

**목표가 조용히 좁혀져 있었다.**
원래 의도는 "FE 코드 퀄리티 향상"인데 문서가 내건 목표는 "검토 가능한 크기
유지"다. 4장에서 선을 긋는 순간 좁아졌는데 명시하지 않았다.
→ P1-4·P1-6 구현. **셀 수 있으면서 퀄리티를 직접 올리는 유일한 영역** —
타입·lint 에러는 취향이 아니라 틀린 코드다. 판단 영역은 훅으로 못 닿는다고
README 에 명시.

**P1 훅 3개 실사용 확인.**
`gate-stop` 은 설정이 없을 때 턴을 정상 종료시켰고, `reinject` 는
**`source: resume` 에서도 발동**했다.
→ 변경 없음. matcher 를 안 건 게 맞았다 — `startup` 만 걸었으면 `--continue`
로 재개한 세션에서 규칙이 빠진다.

**`lint-feedback` 은 이 저장소에서 확인 불가.**
eslint 도 없고 대상 확장자 파일도 없는 셸 스크립트 저장소다.
→ 현업 저장소에 붙여서 확인한다. 공개 기준의 "서로 다른 저장소 2개"와 같이 처리.

**현업 모노레포에 붙여보니 `lint-feedback` 이 세 번 거짓말했다.**
셋 다 "코드에 린트 문제가 있다"고 보고했지만 전부 코드 문제가 아니었다.
① `--no-warn-ignored` 는 ESLint 9 flat config 전용인데 이 저장소는 eslintrc 라
"Invalid option" 으로 죽었다. ② 루트 `.eslintrc.js` 가 워크스페이스 패키지를
참조해 **루트에서는 eslint 가 아예 못 돌았다.** ③ type-aware ESLint 는
tsconfig include 밖의 파일에 "TSConfig does not include this file" 을 낸다.
→ ① 플래그 제거 ② **파일이 속한 패키지 디렉터리에서 실행**(`fh_package_dir`)
③ 해당 에러 문자열은 no-op. 그리고 **exit 1 만 lint 문제로 취급**한다 —
그 외 non-zero 는 린터가 못 돈 것이다.
고친 뒤 같은 저장소에서 `no-unused-vars` 를 정확히 잡는 것까지 확인했다.

**훅이 낼 수 있는 최악의 출력은 차단이 아니라 거짓말이다.**
막히면 사람이 알아채지만, 없는 문제를 고치라고 하면 모델이 조용히 시간을 쓴다.
→ 확신이 없으면 아무것도 안 한다는 6장 원칙을 린트 훅에도 적용했다.
회귀 테스트 2종 추가 (린터 실행 실패 / tsconfig 불일치).

**두 번째 저장소(단일 패키지 Next.js)에서는 전부 한 번에 통과했다.**
`fivespot-frontend` — 포맷(prettier 추론), 린트(`no-unused-vars` 정확히 검출),
컴포넌트 게이트, 분량 게이트, 품질 게이트(설정 없어 무동작) 5종 모두 정상.
모노레포에서 나온 문제 3건이 **모노레포 특유의 문제였다**는 게 확인됐다.
→ 변경 없음. 공개 기준의 "서로 다른 저장소 2개" 충족.
두 저장소 모두 untracked 0 으로 완전 롤백했다.

**문서 기록이 조용히 누락됐다.**
11장 표를 `python .replace()` 로 편집했는데 포매터가 표를 패딩해 문자열이
안 맞았다. `.replace()` 는 못 찾아도 에러 없이 원본을 반환하고, 커밋 전에
확인하지 않아 **"기록했다"고 보고한 항목 12개가 전부 유실**돼 있었다.
→ 이 절을 표에서 목록으로 바꿨다. 그리고 **문서 편집 후에는 반드시
`grep` 으로 반영을 확인**한다.

---

**바꾼 게 있으면 해당 장도 같이 고친다.** 이 목록만 늘어나고 본문이 그대로면
문서가 현실과 어긋나기 시작한다.
