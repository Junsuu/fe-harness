...
## 3. toss/frontend-fundamentals 와의 경계

같이 설치해서 쓴다. **역할이 겹치면 안 된다** — 스킬 본문은 한 번 호출되면 세션 내내 컨텍스트에 남기 때문에, 같은 층위의 규칙을 두 벌 얹으면 비용이 두 배가 되고 충돌 시 Claude가 임의로 하나를 고른다.

실측 결과 (2026-08-17, 저장소 직접 확인):

| | toss/frontend-fundamentals | fe-harness |
|---|---|---|
| 구성 | 스킬 4개(67~88줄) + reviewer 에이전트 + `/review` + eval | **훅 중심** |
| **훅** | **0개** | **핵심** |
| 답하는 질문 | 무엇이 좋은 코드인가 | 언제 강제하고 무엇을 기록하는가 |
| 시점 | **리뷰 시점** (diff 대상, 사후) | **작성 시점** (사전) |
| 층위 | 권고 (스킬·서브에이전트) | 강제 (훅) |
| 수정 권한 | **없음** (`tools: Read, Grep, Glob, Bash, Skill`) | 있음 (`refactor-pass`) |
| eval | 수동 플레이북 (`run-eval.md`) | **자동화 스크립트** ← 차별점 |

**fe-harness는 코드 품질 기준을 의도적으로 정의하지 않는다.** 그건 토스가 한다.
...
(9장 ①이 대표적인 예다) 주차로 묶어봐야 첫 실측에서 무의미해진다.
순서는 10장, 써보고 바꾼 것은 12장에 남긴다.
...
**의도적으로 안 만드는 것**: 코드 품질 원칙 스킬(토스가 함) / 자동 호출 스킬(P0~P2 전부 `disable-model-invocation: true`) / 포맷 이외의 스타일 규칙(린터 영역)
...
| 8 | **그냥 쓰기** | **짜증난 순간 전부 12장에 기록** ← 진짜 산출물 |
...
- [ ] 현업 저장소에서 **실사용** — 안 쓰는 도구는 만든 게 아니다.
      기간이 아니라 **12장에 조정 기록이 쌓이고 새 마찰이 더 안 나오는 것**이 기준이다
...
**이 문서에서 제일 값이 나가는 절이 될 곳이다.** 5장의 임계값도 4장의 반려/경고
